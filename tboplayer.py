...
# ***************************************
    ...
    def init_play_state_machine(self):
        ...
        # signal to break out of Repeat or Playlist loop
    ...
    def play_state_machine(self):
        ...
        # if omxplayer is playing the track change to play state
            if self.omx and self.omx.start_play_signal == True:
                self.monitor(
                    "            <start play signal received from omx")
                ...
                if self.ytdl_state == self._YTDL_CLOSED:
                    self.play_state = self._OMX_CLOSED
                    self.monitor(
                        "      youtube-dl failed, stopping omx state machine")
                else:
            ...
            # or (self.omx and (self.omx.end_play_signal or self.omx.failed_play_signal)):
    ...
    def set_option(self, option, value):
        ...
        if (allowed_option_values == "str" or
            ...
            raise AttributeError(
                "Option value does not match an expected value or pattern")
...
# ******************************************
    ...
    def set_track_position(self, event):
        if not self.dbus_connected:
            return
        new_track_position = self.progress_bar_step_rate * \
            ((event.x * self.progress_bar_total_steps)/event.widget.winfo_width())
        try:
    ...
    def set_progress_bar_step(self):
        try:
            self.progress_bar_var.set(int(
                (self.omx.position * self.progress_bar_total_steps)/self.omx.timenf['duration']))
        except Exception:
            log.logException()
            sys.exc_clear()
...
# ******************************************
    ...
    def create_vprogress_bar(self):
        ...
        self.vprogress_bar_window.protocol(
            "WM_DELETE_WINDOW", self.vprogress_bar_window.destroy)

        self.vprogress_bar_window.video_height = screenres[1]
        self.vprogress_bar_window.video_width = int(
            vsize[0] * (screenres[1] / float(vsize[1])))
        ...
        if self.vprogress_bar_window.video_width > screenres[0] + 20:
            ...
            self.vprogress_bar_window.video_height = int(
                vsize[1] * (screenres[0] / float(vsize[0])))
        ...
        self.vprogress_bar.pack(
            in_=self.vprogress_bar_frame, fill=BOTH, side=BOTTOM)
        self.root.update()

        self.vprogress_bar.bind("<ButtonRelease-1>", self.set_track_position)
        self.vprogress_bar_window.bind("<Configure>", self.move_video)
        self.vprogress_bar_window.bind(
            "<ButtonPress-1>", self.vwindow_start_move)
        self.vprogress_bar_window.bind(
            "<ButtonRelease-1>", self.vwindow_stop_move)
        self.vprogress_bar_window.bind("<B1-Motion>", self.vwindow_motion)
        self.vprogress_bar_window.bind(
            "<Double-Button-1>", self.toggle_full_screen)
    ...
    def vwindow_show_and_hide(self, *event):
        ...
        if self._vwindow_show_and_hide_flag is None:
            self._vwindow_show_and_hide_flag = self.root.after(
                3000, self.vwindow_hide)
        else:
            # refresh timer
    ...
    def toggle_full_screen(self, *event):
        ...
        if self.options.full_screen == 1:
            ...
            vsize_m = self.RE_RESOLUTION.match(
                self.options.windowed_mode_resolution)
            if vsize_m:
    ...
    def destroy_vprogress_bar(self):
        try:
            ...
            sys.exc_clear()
            self.monitor(
                "Failed trying to destroy video window: video window nonexistent.")
    ...
    def save_video_window_coordinates(self):
        ...
        h = self.vprogress_bar_window.winfo_height()
        w = self.vprogress_bar_window.winfo_width()
        self.options.windowed_mode_coords = (
            "+" if x >= 0 else "-")+str(x)+("+" if y >= 0 else "-")+str(y)
        self.options.windowed_mode_resolution = "%dx%d" % (w, h)
        self.monitor('Saving windowed geometry: "%s%s"' % (
            self.options.windowed_mode_resolution, self.options.windowed_mode_coords))
    ...
    def set_volume_bar_step(self, step):
        ...
        if step > self.volume_critical_step:
            self.style.configure(
                "volumebar.Horizontal.TProgressbar", foreground='red', background='red')
...
# ***************************************
    ...
    def display_selected_track(self, index=None):
        ...
        if self.playlist.track_is_selected():
            ...
            self.display_selected_track_title.set(
                self.playlist.selected_track()[PlayList.TITLE])
        else:
            self.display_selected_track_title.set("")
    ...
    def refresh_playlist_display(self):
        ...
        for index in range(self.playlist.length()):
            self.playlist.select(index)
            self.track_titles_display.insert(
                END, self.playlist.selected_track()[PlayList.TITLE])
...
# ***************************************
        ...
        if path:
            ...
            if self.options.last_track_dir != '':
                filez = tkFileDialog.askopenfilenames(
                    initialdir=self.options.last_track_dir, parent=self.root, title=_('Choose the file(s)'))
            else:
                filez = tkFileDialog.askopenfilenames(
                    parent=self.root, title=_('Choose the file(s)'))

        else:
            filez = tkFileDialog.askopenfilenames(
                initialdir=self.options.initial_track_dir, parent=self.root, title=_('Choose the file(s)'))
    ...
    def get_dir(self):
        if self.options.initial_track_dir:
            d = tkFileDialog.askdirectory(
                initialdir=self.options.initial_track_dir, title=_("Choose a directory"))
        elif self.options.last_track_dir:
            d = tkFileDialog.askdirectory(
                initialdir=self.options.last_track_dir, title=_("Choose a directory"))
        else:
    ...
    def youtube_search(self):
        def add_url_from_search(link):
            if self.ytdl.is_running(link):
                return
            if "list=" in link:
                self.go_ytdl(link, playlist=True)
                self.display_selected_track_title.set(
                    _("Wait. Loading playlist content..."))
                return
    ...
    def random_next_track(self):
        if self.playlist.length() > 0:
            index = self.start_track_index = randint(
                0, self.playlist.length()-1)
            self.playlist.select(index)
            self.display_selected_track(index)
...
# ***************************************
    ...
    def clear_list(self):
        if tkMessageBox.askokcancel(_("Clear Playlist"), _("Clear Playlist")):
            self.track_titles_display.delete(
                0, self.track_titles_display.size())
            self.playlist.clear()
            self.blank_selected_track()
            self.display_time.set("")
    ...
    def save_list(self):
        ...
        for idx in range(self.playlist.length()):
            ...
            ofile.write('"' + (item if not 'http' in item else self.playlist.selected_track()[
                        PlayList.LOCATION_BACKUP]) + '","' + self.playlist.selected_track()[PlayList.TITLE]+'"\n')
        ofile.close()
        return
...
class OptionsDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        self.windowed_mode_coords_var = config.get(
            'config', 'windowed_mode_coords', 0)
        self.windowed_mode_resolution_var = config.get(
            'config', 'windowed_mode_resolution', 0)
        self.autolyrics_coords_var = config.get(
            'config', 'autolyrics_coords', 0)
        ...
        rb_hdmi = Radiobutton(master, text=_(
            "HDMI"), variable=self.audio_var, value="hdmi")
        rb_hdmi.grid(row=1, column=0, sticky=W)
        rb_local = Radiobutton(master, text=_(
            "Local"), variable=self.audio_var, value="local")
        rb_local.grid(row=2, column=0, sticky=W)
        rb_auto = Radiobutton(master, text=_(
            "Both"), variable=self.audio_var, value="both")
        rb_auto.grid(row=3, column=0, sticky=W)
        rb_alsa = Radiobutton(master, text="ALSA",
                              variable=self.audio_var, value="alsa")
        ...
        rb_single = Radiobutton(master, text=_(
            "Single"), variable=self.mode_var, value="single")
        rb_single.grid(row=11, column=0, sticky=W)
        rb_repeat = Radiobutton(master, text=_(
            "Repeat"), variable=self.mode_var, value="repeat")
        rb_repeat.grid(row=12, column=0, sticky=W)
        rb_playlist = Radiobutton(master, text=_(
            "Playlist"), variable=self.mode_var, value="playlist")
        rb_playlist.grid(row=13, column=0, sticky=W)
        rb_rplaylist = Radiobutton(master, text=_(
            "Repeat playlist"), variable=self.mode_var, value="repeat playlist")
        rb_rplaylist.grid(row=14, column=0, sticky=W)
        rb_shuffle = Radiobutton(master, text=_(
            "Shuffle"), variable=self.mode_var, value="shuffle")
        ...
        self.youtube_media_format_var.set(
            config.get('config', 'youtube_media_format', 0))
        rb_video = Radiobutton(master, text=_(
            "Video and audio"), variable=self.youtube_media_format_var, value="mp4")
        rb_video.grid(row=18, column=0, sticky=W)
        rb_audio = Radiobutton(master, text=_(
            "Audio only"), variable=self.youtube_media_format_var, value="m4a")
        ...
        self.youtube_video_quality_var.set(
            config.get('config', 'youtube_video_quality', 0))
        om_quality = OptionMenu(
            master, self.youtube_video_quality_var, "high", "medium", "small")
        om_quality.grid(row=21, sticky=W)

        Label(master, text=_("Initial directory for tracks:")).grid(
            row=0, column=2, sticky=W)
        self.e_tracks = Entry(master)
        self.e_tracks.grid(row=1, column=2)
        self.e_tracks.insert(0, config.get('config', 'tracks', 0))
        Label(master, text=_("Inital directory for playlists:")).grid(
        ...
        Label(master, text=_("OMXPlayer location:")).grid(
        ...
        Label(master, text=_("OMXPlayer options:")).grid(
        ...
        self.cb_subtitles = Checkbutton(master, text=_(
            "Subtitles"), variable=self.subtitles_var, onvalue="on", offvalue="off")
        ...
        Label(master, text=_("youtube-dl location:")
              ).grid(row=17, column=2, sticky=W)
        self.e_ytdl_location = Entry(master)
        self.e_ytdl_location.grid(row=18, column=2)
        self.e_ytdl_location.insert(
            0, config.get('config', 'ytdl_location', 0))
        Label(master, text="").grid(row=19, column=2, sticky=W)

        Label(master, text=_("Download actual media URL:")).grid(
            row=20, column=2, sticky=W)
        self.download_media_url_upon_var = StringVar()
        self.download_media_url_upon_var.set(_("when adding URL") if config.get(
            'config', 'download_media_url_upon', 0) == "add" else _("when playing URL"))
        om_download_media = OptionMenu(master, self.download_media_url_upon_var, _(
            "when adding URL"), _("when playing URL"))
        om_download_media.grid(row=21, column=2, sticky=W)

        Label(master, text="").grid(row=22, sticky=W)
        Label(master, text=_("Interface language:")).grid(
            row=23, column=0, sticky=W)
        self.lang_var = StringVar()
        self.lang_var.set(config.get('config', 'lang', 0))
        om_lang = OptionMenu(master, self.lang_var, 'en',
                             'es', 'fr', 'pt', 'pl', 'ro', 'ru')
        om_lang.grid(row=24, column=0, sticky=W)

        Label(master, text="").grid(row=22, sticky=W)
        Label(master, text=_("Subtitles language:")).grid(
            row=23, column=2, sticky=W)
        self.subtitles_lang_var = StringVar()
        self.subtitles_lang_var.set(config.get('config', 'subtitles_lang', 0))
        om_lang = OptionMenu(master, self.subtitles_lang_var, 'ar', 'ch',
                             'de', 'en', 'es', 'fr', 'it', 'ja', 'ko', 'pt', 'pl', 'ro', 'ru')
        om_lang.grid(row=24, column=2, sticky=W)

        self.forbid_windowed_mode_var = IntVar()
        self.forbid_windowed_mode_var.set(
            int(config.get('config', 'forbid_windowed_mode', 0)))
        self.cb_forbid = Checkbutton(master, text=_(
            "Forbid windowed mode"), variable=self.forbid_windowed_mode_var, onvalue=1, offvalue=0)
        ...
        self.cue_track_mode_var.set(
            int(config.get('config', 'cue_track_mode', 0)))
        self.cb_cue = Checkbutton(master, text=_(
            "Begin/End track paused"), variable=self.cue_track_mode_var, onvalue=1, offvalue=0)
        ...
        self.cb_autoplay = Checkbutton(master, text=_(
            "Autoplay on start up"), variable=self.autoplay_var, onvalue=1, offvalue=0)
        ...
        self.cb_ytdl_update = Checkbutton(master, text=_(
            "Keep youtube-dl up-to-date"), variable=self.ytdl_update_var, onvalue=1, offvalue=0)
        ...
        self.cb_find_lyrics = Checkbutton(master, text=_(
            "Find lyrics"), variable=self.find_lyrics_var, onvalue=1, offvalue=0)
        ...
        self.cb_debug = Checkbutton(master, text=_(
            "Debug"), variable=self.debug_var, onvalue='on', offvalue='off')
    ...
    def save_options(self):
        ...
        if (self.lang_var.get() != config.get('config', 'lang', 0)):
            tkMessageBox.showinfo(
                "", _("Restart TBOplayer to change language"))
        ...
        config.set('config', 'youtube_media_format',
                   self.youtube_media_format_var.get())
        config.set('config', 'omx_location', self.e_omx_location.get())
        config.set('config', 'ytdl_location', self.e_ytdl_location.get())
        config.set('config', 'download_media_url_upon', "add" if self.download_media_url_upon_var.get(
        ) == _("when adding URL") else "play")
        config.set('config', 'youtube_video_quality',
                   self.youtube_video_quality_var.get())
        config.set('config', 'geometry', self.geometry_var)
        config.set('config', 'full_screen', self.full_screen_var)
        config.set('config', 'windowed_mode_coords',
                   self.windowed_mode_coords_var)
        config.set('config', 'windowed_mode_resolution',
                   self.windowed_mode_resolution_var)
        config.set('config', 'forbid_windowed_mode',
                   self.forbid_windowed_mode_var.get())
...
class LoadYtPlaylistDialog(tkSimpleDialog.Dialog):
    ...
    def __init__(self, parent):
        ...
        tkSimpleDialog.Dialog.__init__(
            self, parent, _("Load Youtube playlist"))
...
class AutoLyrics(Toplevel):
    _ARTIST_TITLE_REXP = re.compile(
        r"([\w\d.&\\/'` ]*)[-:|~]([\w\d.&\\/'` ]*)", re.UNICODE)
    ...
    def get_lyrics(self, artist, title):
        self._background_thread = Thread(
            target=self._get_lyrics, args=[artist, title])
...
if __name__ == "__main__":
    ...
    try:
        ...
        bus_object = bus.get_object(
            TBOPLAYER_DBUS_OBJECT, TBOPLAYER_DBUS_PATH, introspect=False)
